...
use crate::scene::light::{SceneLight, light_type::{*, self}};
...
impl Rustracer {
	pub fn new() -> Self {
		...
		default_scene
			.lights
			.push(crate::scene::light::SceneLight::Directional(light_type::Directional {
				color: crate::color::RGBA::BLUE,
...
impl Rustracer {
	fn display_editor(&mut self, _ctx: &egui::Context, ui: &mut Ui) {
		...
		ui.horizontal(|ui| {
			...
			self.renderer_dirty |= ui
				.rustracer_color_edit_button_rgba(&mut self.scene.ambiant)
				.changed();
		});
		ui.separator();
	...
	fn display_light_editor_directional(&mut self, ui: &mut Ui, light: &mut Directional) {
		...
		ui.horizontal(|ui| {
			ui.label("Color:");
			self.renderer_dirty |= ui
				.rustracer_color_edit_button_rgba(&mut light.color)
				.changed();
		});
		ui.horizontal(|ui| {
			ui.label("Direction:");
